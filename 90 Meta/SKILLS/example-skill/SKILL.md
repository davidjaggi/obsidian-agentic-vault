# Skill: Process Inbox Note

**When to use**: A new note has landed in `00 Inbox/` and needs to be
triaged.

**Steps**:
1. Read the inbox note in full.
2. Decide where it belongs by actionability, not topic:
   - Has a deadline/outcome → `20 Projects/` (give it its own self-contained
     subfolder if it will accumulate supporting files).
   - An ongoing responsibility, no end date → `30 Areas/`.
   - Reference material with no immediate action → `40 Resources/`.
   - Notes on a book/article/podcast → `40 Resources/Literature/`.
   - A distilled idea in your own words → `50 Notes/`, using
     `90 Meta/Templates/note.md`.
   - A note about a specific person → `50 Notes/`, using
     `90 Meta/Templates/person.md` (`type: person`).
3. Link the new note to at least one related existing note.
4. Delete the original file from `00 Inbox/`.

**Depends on**: `90 Meta/Templates/note.md`, `90 Meta/Templates/person.md`
